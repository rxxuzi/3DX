...
public class DPolygon {
	...
	void updatePolygon() {
		...
		for(int i = 0 ; i < x.length; i ++) {
			newX[i] = dx + Main1_16.ScreenSize.getWidth()  /2 + 50 * Calculator.CalculatorPositionX(Screen.ViewFrom, Screen.ViewTo, x[i], y[i], z[i]);
			newY[i] = dy + Main1_16.ScreenSize.getHeight() /2 + 50 * Calculator.CalculatorPositionY(Screen.ViewFrom, Screen.ViewTo, x[i], y[i], z[i]);
		}
