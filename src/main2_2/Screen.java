...
//	static ArrayList<Ground> Ground = new ArrayList<Ground>();
	...
	public Screen(){		
		...
		/*地面の表示*/
		int d = 2;
		for(int i = -6 ; i < 50 ; i += d) {
			for(int j = -20  ; j < 25 ; j += d) {
				
				if((i+j)/2%2 == 0) {					
					DPolygons.add(new DPolygon(new double[]{i, i, i+d, i+d} , new double[] {j, j+d, j+d, j},new double[] {0,0,0,0}, new Color(190,100,250), false));
				}else {					
					DPolygons.add(new DPolygon(new double[]{i, i, i+d, i+d} , new double[] {j, j+d, j+d, j},new double[] {0,0,0,0}, new Color(250,100,190), false));
				}
			}
		}
...
//		Cube.get(0).updatePoly();
		...
		g.drawString("Number Of Polygons : " + Arrays.toString(FViewFrom) , 10 ,150);
		g.drawString("Focus Polys " + FocusPolygon.toString() , 20 ,170);
		if(NumberOfDeleteCube > 0) g.drawString("Most recently deleted: " + dCube , 20 ,190);
		
		//描画更新のインターバル
		SleepAndRefresh();
	}
	
	//Keys7が押された時、フォーカスしているキューブを削除する
	private void deleteCube() {
		if(Keys[7]) {
			//インターバルを測定
			if(System.currentTimeMillis() - LastCubeDeleteTime >= deleteinterval) {
				//フォーカスしているポリゴンから属しているキューブを特定
	...
	class KeyTyped extends KeyAdapter{
		...
		public void keyPressed(KeyEvent e) {
			switch(e.getKeyCode()) {
			case KeyEvent.VK_W : 	 Keys[0] = true ; break; //前進
			case KeyEvent.VK_A : 	 Keys[1] = true ; break; //後退
			case KeyEvent.VK_S : 	 Keys[2] = true ; break; //左へ
			case KeyEvent.VK_D :	 Keys[3] = true ; break; //右へ
			case KeyEvent.VK_Q : 	 Keys[4] = true ; break; //視点リセット
			case KeyEvent.VK_SPACE:	 Keys[5] = true ; break; //上へ
			case KeyEvent.VK_SHIFT:	 Keys[6] = true ; break; //下へ
			case KeyEvent.VK_DELETE: Keys[7] = true ; break; //下へ
		...
		public void keyReleased(KeyEvent e) {
			...
			case KeyEvent.VK_DELETE: Keys[7] = false ; break;
